...
func TestDB_Put(t *testing.T) {
	opts := DefaultOptions
	opts.DataFileSize = 1 * 1024 * 1024 // 1 MB
	db, err := Open(opts)
	defer destroyDB(db)
	assert.Nil(t, err)
	assert.NotNil(t, db)
...
func TestDB_FileLock(t *testing.T) {
	...
	// db2 can't be opened before db1 is destroyed
	db2, err := Open(opts)
	assert.Nil(t, err)
	assert.NotNil(t, db2)
	err = db2.Close()
...
func TestDB_Stat(t *testing.T) {
	opts := DefaultOptions
	opts.DataFileSize = 1024 * 1024 // 1mb
	...
	for i := 1; i < 15*1024; i++ {
		err := db.Put(utils.GetTestKey(i), utils.RandomValue(1024)) // 1kb
		assert.Nil(t, err)
	}
	for i := 1; i < 5*1024; i++ {
	...
	stat := db.Stat()
	assert.Equal(t, stat.KeyNum, uint(10*1024))
	assert.Greater(t, stat.DataFileNum, uint(20))
	assert.Greater(t, stat.ReclaimableSize, int64(10*1024*1024))
	assert.NotNil(t, stat)
	t.Log(fmt.Printf("KeyNum:%v, DataFileNum:%v, ReclaimableSize:%v, OccupiedDiscSize:%v",
		stat.KeyNum, stat.DataFileNum, stat.ReclaimableSize, stat.OccupiedDiscSize))
}
...
func TestDB_Backup(t *testing.T) {
	...
	backupDir, _ := os.MkdirTemp("", "kv-backup")

	err = db.Backup(backupDir)
	assert.Nil(t, err)

	opts2 := DefaultOptions
	opts2.DataFileSize = 1024 * 1024
...
func TestDB_OpenDatafiles(t *testing.T) {
	opts := DefaultOptions
	opts.DirPath = "/tmp/DB-mmap"
	db, err := Open(opts)
	assert.Nil(t, err)
	assert.NotNil(t, db)
	//  DataFileSize:        256 * 1024 * 1024, (256MB)
	// generate test data more than 2 GB
	for i := 1; i < 20*256*1024; i++ {
		err := db.Put(utils.GetTestKey(i), utils.RandomValue(1024)) // 1kb
		assert.Nil(t, err)
	}

	opts2 := DefaultOptions
	opts2.DirPath = "/tmp/DB-standard"
	db2, err2 := Open(opts2)
	assert.Nil(t, err2)
	assert.NotNil(t, db2)
	//  DataFileSize:        256 * 1024 * 1024, (256MB)
	// generate test data more than 2 GB
	for i := 1; i < 20*256*1024; i++ {
		err2 := db2.Put(utils.GetTestKey(i), utils.RandomValue(1024)) // 1kb
		assert.Nil(t, err2)

	}
}

func TestDB_OpenWithMMap(t *testing.T) {
	// open with MMap
	opts1 := DefaultOptions
	opts1.DirPath = "/tmp/DB-mmap"
	opts1.MMapAtStartupNeeded = true

	now := time.Now()
	db, err := Open(opts1)
	//defer destroyDB(db)
	t.Log("opening a db with MMap IO costs:", time.Since(now))
	assert.Nil(t, err)
	assert.NotNil(t, db)
	...
	opts2 := DefaultOptions
	opts2.DirPath = "/tmp/DB-standard"
	opts2.MMapAtStartupNeeded = false
	now2 := time.Now()
	db2, err2 := Open(opts2)
	//defer destroyDB(db2)
	t.Log("opening a db with standard IO costs:", time.Since(now2))
	assert.Nil(t, err2)
	assert.NotNil(t, db2)
}
