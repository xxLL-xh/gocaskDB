...
func (rds *Redis) ZAdd(key []byte, score float64, member []byte) (bool, error) {
	...
	if exist {
		...
		_ = wb.PendingDelete(oldKey.encodeWithScore())
	}
	_ = wb.PendingPut(zk.encodeWithMember(), utils.Float64ToBytes(score))
	_ = wb.PendingPut(zk.encodeWithScore(), nil)
...
func (rds *Redis) ZScore(key []byte, member []byte) (float64, error) {
	meta, err := rds.findMetadata(key, ZSet)
	if err != nil {
		return -1, err
	}
